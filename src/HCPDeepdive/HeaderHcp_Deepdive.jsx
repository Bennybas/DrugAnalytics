...
const HeaderHcp_Deepdive = ({onAccChange,Hcp_Id}) => {
    const [isOpen, setIsOpen] = useState(false);
      const [selectedAccount, setSelectedAccount] = useState(Hcp_Id || null);
      const dropdownRef = useRef(null);
    
      // Update selected account when Hcp_Id prop changes
      useEffect(() => {
        if (Hcp_Id) {
          setSelectedAccount(Hcp_Id);
        }
      }, [Hcp_Id]);
                    ...
                    {Accounts.map((account, index1) => (
                      <div
                        ...
                        className={`px-4 py-2 text-sm cursor-pointer transition-colors duration-150
                          ${
                            (selectedAccount === account.id || Hcp_Id === account.id)
...
export default HeaderHcp_Deepdive